#!/usr/bin/env node
/**
 * Quick Stripe readiness check — run after pasting your keys into backend/.env
 * Usage: node backend/scripts/stripe-status.mjs
 */
...
console.log(`  backend/.env       ${existsSync(envPath) ? "found" : "MISSING"}`);
...
if (mock) {
  console.log("\nNext steps:");
  console.log("  1. Paste your Stripe test keys into backend/.env");
