...
/**
 * Present native Stripe PaymentSheet for a booking checkout response.
 * Falls back when native Stripe is unavailable (web / Expo Go without native module).
 */
export async function presentBookingPayment(
  ...
  // PaymentSheet is native-only; skip on web.
  if (Platform.OS === "web") {
    return {
      status: "unavailable",
